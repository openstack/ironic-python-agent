...
def get_latest_coreos_version():
    r = requests.get(COREOS_STABLE_URL)
    for line in r.text.split('\n'):
        if '=' in line:
            k, v = line.split('=')
            if k == 'COREOS_VERSION_ID':
                return v
    raise ValueError('Could not fetch latest CoreOS stable version')


COREOS_STABLE_URL = ('http://stable.release.core-os.net/amd64-usr'
                     '/current/version.txt')
COREOS_VERSION = get_latest_coreos_version()
